...
py -3 migrate.py import --claude <c.zip> --remap-path "<repo-root>\Desktop\proj" "D:\work\proj"  # 工程挪到别的文件夹
```
...
- 用户名改写(两者都支持):新机用户名不同时用 `--remap-user OLD NEW`。
  ...
  - Codex:改写 `sessions/` 文本文件,以及 `*.sqlite` 里 `rollout_path`/`cwd`/`agent_path` 等文本列(走 SQL `UPDATE...REPLACE`,不是裸字节替换——后者会撑坏 SQLite 记录导致损坏)。Codex 的 `rollout_path` 是绝对路径,用户名变了不 remap 就找不到会话文件,所以这步是必要的。
  - 两者都只改 `Users<分隔符>用户名<分隔符>` 形态,不误伤同名词,也不碰非 Users 路径(如 D 盘项目)。
- 工程路径搬迁(仅 Claude):用 `--remap-path OLD_PATH NEW_PATH`。Claude 的聊天记录/记忆
  目录是按工程绝对路径编码命名的(`[^A-Za-z0-9]→-`),工程挪到别的文件夹后,新路径下
