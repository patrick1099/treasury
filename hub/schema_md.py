...
#### 提取器不认识的键:**原样带着走,一个都不丢**

上面那 7 个字段(`name` / `description` + `metadata` 的 4 个)是提取器**会去判断**的
全部字段。除此之外的键——比如 Claude 自己写进 `metadata` 的 `originSessionId`(记忆
出生的那次会话)和 `node_type`——提取器**看不懂,但照样原样搬进金库、原样吐回来**。

> **看不懂 ≠ 不重要。** 看不懂只意味着**不归提取器管**;不归它管的东西,它更没有资格丢。
> 备份区的立身之本是"别丢"——一份把字段悄悄削掉的备份,不叫备份。

已知的 7 个字段在文件里位置固定、写法固定;不认识的键跟在各自那一层的已知键后面,
顺序沿用源文件。**对 C 的意义**:金库里的记忆**保留**了工具自己的元数据字段,
你可以放心依赖它们(例如靠 `originSessionId` 把记忆追回它出生的那次会话)。
...
- `class:<名>` 对的是 `device.toml` 的 `class` 数组;`project:<名>` 对的是 `projects` 数组。
  `project:projx` 是**设备订阅条件**(本机 projects 含 projx 才纳入),**不是**"仅 projx
  工程会话可见"——视图是**用户级全局视图**。
...
同一条记忆要在多台机器上用,而 `C:\\\\Users\\\\<user>\\\\...` 换台机就是死链。所以正文里
...
- **一台新机怎么加进来**:`git clone` 那个金库,再跑一次 `hub-scaffold <金库> <本机名>`。
  它**新增** `<本机名>/` 那棵子树和缺失的根文件,**不需要 `--force`**;`vault.toml` 和
  `lint-exempt.txt` **已存在就绝不覆盖**,别的设备的目录碰都不碰。
  (`<本机名>/device.toml` 已经存在时它会**拒绝**:那是本机档案,覆盖就是把你手填的
  源路径换回占位符。这时候你要的多半是 `hub collect`,不是重新建库。)

  **唯一的例外是 `SCHEMA.md`(就是本文件):它每次都被无条件重写。** 这是有意的——它是从
  提取器代码里派生出来的契约文本,金库里应该永远是"当前这个版本的提取器"所描述的契约,
  而不是建库那天的历史快照。**"已有内容原样保留"这条对它不成立**,别指望在它里面手写笔记。

  > ⚠ **由此带来一个真实的坑,加载器作者必须知道**:如果 B 机上装的是**更老**的 hub,
  > 它 scaffold 进这个 clone 下来的金库时,会把 `SCHEMA.md` **静默降级**成老版本的契约
  > ——而 A 机的加载器可能正依赖新版本里才有的约定。金库里没有任何东西会提醒你这件事。
  > 防法只有一条:**各设备的 hub 版本保持一致**(尤其别拿老机器往新金库里 scaffold);
  > 真出了怪事,先 `git log -- SCHEMA.md` 看看它是不是被谁改回去了。
