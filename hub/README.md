...
| `inventory` | 输出本机资产×harness 矩阵(plugin / skill / memory;`--json` 信封) | 否 |
| `toggle <asset_type> <name> <harness> <on\|off> [--plugin P] [--dry-run]` | 手动开关/清理;`plugin` 四个 harness 均可,`memory` 只读拒绝 | 否 |
...
同一文件里的 `[skills.<tool>].enabled` 表达独立 skill 与插件内 skill 的本机期望态：缺省没有该 key = 全部默认开启；
插件内 skill 还要所属插件开启才生效。`opencode` / `dsh` 支持 skill 级真过滤，`claude` / `codex`
的插件内 skill 仍由插件整体控制。可用 `hub inventory --json` 只读查看矩阵，用 `hub toggle` 手动开关。
