...
**Interfaces:**
- Produces: `class ConflictError(RuntimeError)`；`class Backend(ABC)` 三方法 `acquire()`/`publish(message:str)`/`status() -> str`；`class GitBackend(Backend)`（`__init__(self, repo:Path)`；`acquire`=`git pull --ff-only`，冲突/非 ff 抛 `ConflictError`；`publish`=`add -A` + `commit` +（若有 remote）`push`；`status`=`git status --porcelain`）。
...
class GitBackend(Backend):
    ...
    def acquire(self) -> None:
        if not self._has_remote():
            return
        r = self._run("pull", "--ff-only", check=False)
        if r.returncode != 0:
            raise ConflictError(
                f"git pull --ff-only 失败(可能分叉/冲突):\n{r.stderr or r.stdout}")
...
**Interfaces:**
- Consumes: 全部前置模块。
- Produces: `main(argv:list[str]) -> int`；子命令 `status` / `collect` / `process` / `pull` / `sync` / `bootstrap`。以 `--vault <path>` 指定本地金库 clone，`--host <name>` 覆盖主机名（默认 `current_host()`）。`process` = collect 结果基础上重生成 `MEMORY.md` + lint（scope/裸路径）+ 本地 commit。`pull` = `acquire` + materialize 到设备 targets。`sync` = `acquire`（冲突即停并返回非零）+ `publish`。
...
def _cmd_collect(args) -> int:
    print("collect 源目录需按设备配置提供；MVP CLI 暂以 process/pull 为主")
    return 0
...
- Produces:
  - `write_claude_index(memories:list[Memory], paths:dict[str,str], claude_home:Path) -> Path`（把 `tool=claude` 选中的记忆渲染成 bundle，写 `claude_home/hub/memory-index.md`，返回该路径）；
  - `write_codex_user_memories(memories:list[Memory], paths:dict[str,str], codex_mem_dir:Path) -> list[str]`（把 `tool=codex` 选中的**用户级**记忆——即 scope 不含 `project:` 维度的——逐条整文件写入 `codex_mem_dir`，返回写入 name 列表）。
...
def test_write_claude_index_bundle(tmp_path):
    ...
    out = write_claude_index(mems, {"VAULT": "Z:/v"}, home)
...
def test_write_codex_user_skips_project_scoped(tmp_path):
    ...
    written = write_codex_user_memories(mems, {}, d)
...
def write_claude_index(memories: list[Memory], paths: dict[str, str],
                       claude_home: Path) -> Path:
    target = Target(frozenset(), None, "claude")
...
def write_codex_user_memories(memories: list[Memory], paths: dict[str, str],
                              codex_mem_dir: Path) -> list[str]:
    target = Target(frozenset(), None, "codex")
...
```python
    ...
    if claude_home:
        write_claude_index(vault.memories, dev.paths, Path(claude_home))
    codex_home = dev.paths.get("CODEX_HOME")
    if codex_home:
        write_codex_user_memories(vault.memories, dev.paths, Path(codex_home) / "memories")
```
Run: `py -3 -m pytest tests/hub -v`
Expected: 全绿（约 29 passed）。
