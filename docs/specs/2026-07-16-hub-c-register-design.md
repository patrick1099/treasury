...
- **P4 启用态 = `<host>/device.toml` 的 `[plugins.<tool>] enabled` 允许列表**（设备×平台维度；manifest **不含**
  ...
  - **Codex**：列表内 → 确保 `add`；列表外**已装** → `remove`（Codex 的 enabled 列表即"期望安装集合"，
    无"已装但禁用"态）。
  - 市场：register 注册全部适配本平台的 market-of-one（幂等，已注册则跳）。
- **P5 refresh（cachebuster 感知，主路径；用户拥有 bump，refresh 不写插件仓；不改变启用/安装策略）**。**先读该平台
  ...
  | **SHA 未变** | **no-op**（幂等） |
  | **SHA 变 & version 变** | CLI 重装 + 更新**该平台**台账 |
...
- **P8 一次性迁移/induction（Plan 3 首批任务，用 C4 通用原语）**：把现有 6 仓从 `~/.claude/plugins-dev` induction
  ...
  - **`cjt` 类（身份不变、只换 source 路径）**：本机实查 `cjt` 市场已注册、身份已是 `cjt@cjt`，但 source 仍指
    ...
    `<vault>/shared/plugins/cjt`。→ **同名市场换源 spike（P8 内先验，写进首批技术任务）**：
    `claude/codex plugin marketplace add` 对**已注册的同名**市场是**覆盖 / 拒绝 / 还是必须 remove→add→reinstall**？
    真机验清再定迁移步骤，**不假设 CLI 会静默覆盖**。

> **实现期 spike（Plan 3 首个技术任务，别靠假设设计）**：**Codex 侧明确**——bump manifest 版本 + `codex plugin add`
> 重读本地市场快照。**Claude 侧待验**——本地 directory 市场靠哪条命令真正重读新内容（`plugin install` 重装是否重读
> 目录？还是必须 version 变？`marketplace` 是否要先刷新？）。真机拿 true-north 一次性验掉，再定 refresh 的 Claude 重装命令。
> 另：compact-plus 含 **hook**，Codex 有 `trusted_hash` 信任闸（内容变要重信任）——hooks 本就降出 v1，此边缘只**记录**、不处理。
