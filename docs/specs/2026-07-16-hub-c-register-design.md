...
- **本会话未真机验证**：Claude/Codex junction 冒烟已通过（Task 0，用户手动）；opencode 因
  `~/.config/opencode` EEXIST 非阻断跳过；临时 junction 已清。仍未验证：受管块 `@import` 是否被
  Claude 真正加载（hub 写块已测，但 Claude 运行时是否吃这段 `@import` 未真机确认）、
  `autoMemoryDirectory` 读写、多机 sync。实现前/中要真机确认。
