...
- **P8 一次性迁移/induction（Plan 3 首批任务，用 C4 通用原语）**：把现有 6 仓从 `~/.claude/plugins-dev` induction
  ...
  - **三段式迁移顺序（缺陷 dcea199 修复，2026-07-22）——切指向前绝不删旧市场根**：
    1. **`migrate-plugins`(phase1)**：只 copy + 校验 + induct，**绝不删源**。旧市场根保留，cutover 前 Codex
       始终能读到它、不会整体悬空崩。重跑允许 src+dest 共存：内容+Git 身份一致→幂等只 induct；不同→冲突拒绝、零删除。
    2. **`cutover-plugins`(phase2)**：只官方 CLI 平台切换，不删文件源。
    3. **`retire-plugin-sources`(phase3，独立命令)**：全量预检两平台已无旧 marketplace/source/身份引用（读不到
       平台状态也拒绝）+ 新身份均装且启用策略正确；任一失败→零删除；只删迁移输入声明的旧子仓，不碰外层容器及独有 docs；
       `--dry-run` 与真跑共用 planner/executor，删除前保留人工闸。**不得用「枚举失败时从 config 猜状态」当修复。**
